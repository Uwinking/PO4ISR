...
class Select():
    ...
    def ucb(self, prompt_list):
        print("prompt_listlen:",len(prompt_list))
        ...
        for t in range(1, self.config['time_steps']+1):
            ...
            select_prompt_reward = asyncio.run(self.reward_model.calculate_reward(select_prompt, sample_data))
            ...
            elapsed_time = time.time() - start_time  # 计算请求总耗时
        ...
        # 选择实际的 beam 宽度：不超过 prompt_list 的长度
        beam_width = min(self.config['beam_width'], len(prompt_list))
        ...
        # 取出前 beam_width 条提示词
        # top_b_prompt = [pair[1] for pair in pairs[:beam_width]]
        
        
        return pairs
    
    def run(self, prompt_list):
        pairs = self.ucb(prompt_list)
